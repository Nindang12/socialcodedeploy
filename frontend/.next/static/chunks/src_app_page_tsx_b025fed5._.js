    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "Post.useEffect": ()=>{
            const checkFollowStatus = {
                "Post.useEffect.checkFollowStatus": async ()=>{
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            const isUserFollowing = followers.map({
                                "Post.useEffect.checkFollowStatus.isUserFollowing": (id)=>String(id).trim()
                            }["Post.useEffect.checkFollowStatus.isUserFollowing"]).includes(String(currentUser.user_id).trim());
            ...
            }["Post.useEffect.checkFollowStatus"];
            checkFollowStatus();
        }
    }["Post.useEffect"], [
        token,
    ...
    const handleFollow = async ()=>{
        ...
        try {
            ...
            if (checkResponse.ok) {
                ...
                const alreadyFollowing = followers.map((id)=>String(id).trim()).includes(String(currentUser.user_id).trim());
                if (alreadyFollowing) {
    ...
    const handleUnfollow = async ()=>{
        ...
        try {
            ...
            if (checkResponse.ok) {
                const userData = await checkResponse.json();
                const followers = userData.user?.followers || [];
                const isFollowing = followers.map((id)=>String(id).trim()).includes(String(currentUser.user_id).trim());
                if (!isFollowing) {
...
_s1(Post, "N4R/4YvI9hSBNBgYlijajHxB9r8=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$navigation$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRouter"]
    ];
});
